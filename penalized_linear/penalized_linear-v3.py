...
if __name__ == "__main__":
    ...
    while (starting + pd.DateOffset(years=11 + counter)) <= pd.to_datetime("20240101", format="%Y%m%d"):
        ...
        gb_bayes = BayesSearchCV(GradientBoostingRegressor(random_state=42), gb_params, scoring='neg_mean_squared_error', n_iter=20, cv=3, n_jobs=-1, random_state=42)
    ...
    out_path = os.path.join(work_dir, "output-v3.csv")
