...
### 1. **R² 的定义**
决定系数 \( R^2 \) 用来评估模型预测值与真实值之间的拟合程度，其公式为：
\[
...
### 2. **计算步骤**
1. **使用 LightGBM 模型生成预测值**：
   - 使用训练好的模型对测试集或验证集进行预测。
2. **调用 `r2_score` 函数计算 \( R^2 \)**：
   - `r2_score` 接受真实值和预测值作为输入。

---

### 4. **输出解释**
- \( R^2 \) 的取值范围通常为 \( (-\infty, 1] \)：
  - \( R^2 = 1 \)：模型的预测值与真实值完全一致，拟合效果最好。
  - \( R^2 > 0 \)：模型比简单的均值预测更准确。
  - \( R^2 = 0 \)：模型的预测效果与简单的均值预测相当。
  - \( R^2 < 0 \)：模型的预测效果比简单的均值预测还差。

---

### 5. **注意事项**
1. **模型的预测范围**：
   - LightGBM 的预测值通常是连续值，确保目标变量的分布与模型预测值一致。
   
2. **数据分割**：
   - 计算 \( R^2 \) 时，建议对 **测试集** 或 **验证集** 计算，而不是训练集，以避免过拟合的影响。

3. **非线性关系**：
   - 如果目标变量与特征之间存在非线性关系，可能需要调整特征或模型参数以提升 \( R^2 \)。
